...
const runners = [
  ...
  // Problem 1: 
  // .filter() -- return the even/odd numbers in an array containing numbers
  function findEvenAndOdd(array) {
    let evenNums = array.filter(number => typeof number === 'number' && number % 2 === 0);
    let oddNums = array.filter(number => typeof number === 'number' && number % 2 !== 0);
    return {evenNums, oddNums};
  }

  const nums = [1,2,3,'bananas',4,5,'apricot',6,'apples',7,8,9,10];
  console.log(findEvenAndOdd(nums));
  
  
  // Problem 2:
  // .map() -- element-wise multiplication of two number arrays
  function elementMultiply(arr1, arr2) {
    if(arr1.length === arr2.length) {
      return arr1.map((num, ind) => num * arr2[ind]);
    } else {
      return "Arrays are not of matching dimensions!";
    }
  }

  const numSet1 = [1, 2, 3, 3];
  const numSet2 = [5, 4, 2, 1];
  console.log(elementMultiply(numSet1, numSet2)); // [5, 8, 6, 3]


  // Problem 3:
  // .forEach() -- reverse a string
  ...
  console.log(reverseString('fork'));